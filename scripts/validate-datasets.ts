...
const tech: TechSidecar | null = fs.existsSync(SIDECAR_PATH)
  ? (JSON.parse(fs.readFileSync(SIDECAR_PATH, 'utf8')) as TechSidecar)
  : null;
...
// ---- Technical validation section (from the link-checker sidecar) ----
const buildTechnicalSection = (): string => {
  if (!tech) {
    return `## Technical validation (URL health & checksums)

_Not run yet._ Run \`npm run check:links\` (optionally with \`-- --checksum\`) to
generate real URL-health and checksum data. Results are written to
\`docs/dataset-technical-validation.json\` and folded in here on the next
\`npm run validate:datasets\`.`;
  }

  const t = tech.totals;
  const fileResults = tech.results.filter((r) => r.role !== 'image');
  const idsByStatus = (status: string): string[] =>
    [...new Set(fileResults.filter((r) => r.status === status).map((r) => r.datasetId))];

  const adminReview = [
    ...new Set(
      fileResults
        .filter((r) => ['broken', 'forbidden', 'not-found', 'cors-limited', 'unknown'].includes(r.status))
        .map((r) => r.datasetId)
    ),
  ];

  const statusLine = (label: string, status: string): string => {
    const ids = idsByStatus(status);
    return `- **${label}:** ${t[status] ?? ids.length}${ids.length ? ` — ${mdList(ids)}` : ''}`;
  };

  return `## Technical validation (URL health & checksums)

> Generated by \`scripts/check-dataset-links.ts\` on ${tech.generatedAt}
> (checksums ${tech.checksumEnabled ? 'enabled' : 'disabled'}${tech.partial ? ', partial run' : ''}).
...
- **URLs checked:** ${t.urlsChecked} (${t.fileUrlsChecked} file URLs + images)
${statusLine('Available', 'available')}
- **Broken:** ${t.broken}${idsByStatus('broken').length ? ` — ${mdList(idsByStatus('broken'))}` : ''}
- **Forbidden:** ${t.forbidden}${idsByStatus('forbidden').length ? ` — ${mdList(idsByStatus('forbidden'))}` : ''}
...
const technicalSection = buildTechnicalSection();
