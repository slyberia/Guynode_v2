...
const MANIFEST_PATH = path.resolve(ROOT, 'data/geojsonManifest.ts');
...
const writeManifest = (entries: Record<string, ManifestEntry>): void => {
  ...
  const body = keys
    .map((k) => `  ${JSON.stringify(k)}: { bytes: ${entries[k].bytes}, features: ${entries[k].features}, simplify: ${JSON.stringify(entries[k].simplify)} },`)
    .join('\n');
  const ts = `// AUTO-GENERATED by scripts/convert-shapefiles.ts — do not edit by hand.
...
const main = async () => {
  ...
  // Rebuild the manifest from what exists on disk for ALL shapefile records, so
  // a subset run does not drop previously-converted entries.
  ...
  for (const rec of records.filter((r) => r.format === 'Shapefile')) {
    ...
    if (fromThisRun && fromThisRun.status === 'converted') {
      manifest[fromThisRun.url] = { bytes: fromThisRun.bytes, features: fromThisRun.features, simplify: fromThisRun.simplify };
      convertedUrls.set(rec.id, fromThisRun.url);
      continue;
    }
    // Not converted this run: keep an existing on-disk file if present.
    const slug = slugForCategory(rec.category);
    const url = `/data/${slug}/${rec.id}.geojson`;
    const file = path.join(PUBLIC_DATA_DIR, slug, `${rec.id}.geojson`);
    if (fs.existsSync(file)) {
      const raw = fs.readFileSync(file, 'utf8');
      manifest[url] = { bytes: Buffer.byteLength(raw, 'utf8'), features: (JSON.parse(raw).features || []).length, simplify: 'existing' };
      convertedUrls.set(rec.id, url);
    }
  }
